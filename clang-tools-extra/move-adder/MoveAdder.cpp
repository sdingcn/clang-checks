...
class MoveAdderConsumer : public clang::ASTConsumer {
public:
  explicit MoveAdderConsumer(ASTContext *context, Rewriter &r) {}
...
class MoveAdderAction : public clang::ASTFrontendAction {
public:
  virtual std::unique_ptr<clang::ASTConsumer>
  CreateASTConsumer(clang::CompilerInstance &compiler,
                    llvm::StringRef inFile) override {
    rewriter.setSourceMgr(compiler.getSourceManager(), compiler.getLangOpts());
    return std::make_unique<MoveAdderConsumer>(&compiler.getASTContext(), rewriter);
  }
...
int main(int argc, const char **argv) {
  ...
  auto retval = Tool.run(newFrontendActionFactory<MoveAdderAction>().get());
  auto endTime = std::chrono::steady_clock::now();
  std::chrono::duration<double> secDiff = endTime - startTime;
  llvm::outs() << "Time used (seconds): " << format("%.3f", secDiff.count()) << "\n\n";
  return retval;
}